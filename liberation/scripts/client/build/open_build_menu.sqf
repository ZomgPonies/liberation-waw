private [ "_oldbuildtype", "_cfg", "_initindex", "_dialog", "_iscommandant", "_squadname", "_buildpages", "_build_list", "_classnamevar", "_entrytext", "_icon", "_affordable", "_affordable_crew", "_selected_item", "_linked", "_linked_unlocked", "_base_link", "_link_color", "_link_str", "_player_AI" ];
...
while { dialog && alive player && (dobuild == 0 || buildtype == 1)} do {
	...
	if (_oldbuildtype != buildtype || synchro_done ) then {
		...
		lbClear 110;
		{
			...
			if ( buildtype != 8 ) then {
				...
				if ( _classnamevar == "LIB_opelblitz_fuel" ) then {
					_entrytext = "Opel Blitz (Repair and Refuel)";
				};
				if ( _classnamevar == "Land_Workbench_01_F" ) then {
					_entrytext = "Vehicle Repair Workbench";
				};
	...
	_affordable = false;
	_squad_full = false;
	_player_AI = GRLIB_max_squad_size;
	if ((player isKindof "LIB_GER_hauptmann") or (player isKindof "LIB_GER_oberst")) then {_player_AI = 30;};
	if (player isKindof "LIB_GER_unterofficer") then {_player_AI = 10;};
	if ((buildtype == 1) && (count (units group player) >= _player_AI)) then {
		_squad_full = true;
	};
	_linked = false;
