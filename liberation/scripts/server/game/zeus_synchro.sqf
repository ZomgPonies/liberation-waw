...
while { true } do {
	...
	_zeusunits = [];
	{
		if ( (side group _x == GRLIB_side_friendly ) && ( _x distance lhd > 200 ) && alive _x ) then {
			_zeusunits pushback _x;
		};
	} foreach allUnits;

	{
		if ((typeof _x in _vehicleClassnames ) && (typeof _x != ammobox_o_typename) && (( _x distance lhd > 100 ) || (typeof _x == huron_typename)) && alive _x ) then {
			_zeusunits pushback _x;
		};
	} foreach vehicles;
